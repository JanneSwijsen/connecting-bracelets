## Overzicht van Design Requirements
|| **Status** || ❌ niet voldaan || ⚠️ gedeeltelijk voldaan || ✅ 


| ID | Design Requirement | Source | Status |
|----|-------------------|--------|--------|
| **Algemeen** |
| 1.1 | Het product moet kunnen navigeren. | Interview | ✅ |
| 1.2 | Het product moet speelfuncties bevatten. | Interview | ⚠️ |
| 1.3 | De functies op de armbanden zijn aangepast aan alle types van blindheid. | Interview | ❌ |
| 1.4 | Elke armband bevat een specifieke kleur. |  | ❌ |
| 1.5 | De verschillende kleuren van de armband moeten goed te onderscheiden zijn. |  | ❌ |
| 1.6 | Het product geeft feedback na elke functie die werd uitgevoerd. | Interview | ❌ |
| 1.7 | De functies op de armbanden zijn aangepast aan kinderen met een fysieke beperking. | Interview | ❌ |
| **Navigatie** |
| 2.1 | Het product moet duidelijke signalen geven in de vorm van trillingen. | Concept Testing | ✅ |
| 2.2 | Het product moet op een zeer nauwkeurige manier de weg leiden. |  | ❌ |
| 2.3 | De trillingen moeten op een constant ritme worden doorgestuurd. | Concept Testing | ✅ |
| 2.4 | De trillingen mogen niet te snel na elkaar volgen. | Concept Testing | ✅ |
| 2.5 | Het moet duidelijk zijn voor de gebruiker wanneer hij links of rechts moet gaan. | Concept Testing | ✅ |
| **Speelfunctie** |
| 3.1 | Het product moet auditieve feedback geven. | Concept Testing | ⚠️ |
| 3.2 | De feedback moet duidelijk en verstaanbaar zijn. | Concept Testing | ⚠️ |
| 3.3 | Het volume moet aangepast kunnen worden. |  | ❌ |
| 3.4 | De speelfunctie moet de gebruiker motiveren om te blijven bewegen. |  | ❌ |
| **Gebruiksgemak** |
| 4.1 | Het product moet gemakkelijk aan en uit te zetten zijn. |  | ❌ |
| 4.2 | Het product moet intuïtief te gebruiken zijn. |  | ❌ |
| 4.3 | Het product moet draadloos kunnen functioneren. | Concept Testing | ⚠️ |
| 4.4 | De armband moet gemakkelijk om de pols te bevestigen zijn. |  | ❌ |
| 4.5 | Het product moet gebruikt kunnen worden door kinderen van verschillende leeftijden. | Concept Testing | ⚠️ |
| 4.6 | Het product moet te gebruiken zijn voor kinderen met een looprek. |  | ❌ |
| 4.7 | De levensduur van het product mag niet te kort zijn. |  | ❌ |
| 4.8 | De armband moet makkelijk op te laden zijn. |  | ❌ |
| 4.9 | De volumestand moet regelbaar zijn. |  | ❌ |
| **Veiligheid** |
| 5.1 | De armband moet weerbestendig zijn. |  | ❌ |
| 5.2 | De motoren mogen niet opwarmen tijdens gebruik. |  | ❌ |
| 5.3 | De armband mag geen scherpe hoeken bevatten. |  | ❌ |
| 5.4 | De armband moet uit taaie materialen bestaan. |  | ❌ |
| **Ergonomie** |
| 6.1 | De armband moet comfortabel rond de pols zitten. |  | ❌ |
| 6.2 | De armband moet instelbaar zijn in polsmaat. |  | ❌ |
| 6.3 | De armband moet stevig rond de pols zitten tijdens fysieke bewegingen. |  | ❌ |
| 6.4 | Het product mag niet te zwaar zijn. |  | ❌ |



